...
prefix = /opt/risc32_self_compile
exec_prefix = ${prefix}
...
TOPLEVEL_CONFIGURE_ARGUMENTS=./configure --prefix=/opt/risc32_self_compile/ --with-arch=rv32im --enable-languages=c
...
BUILD_CONFIGARGS =  --cache-file=./config.cache '--prefix=/opt/risc32_self_compile/' '--with-arch=rv32im' '--enable-languages=c' --program-transform-name='s,y,y,' --disable-option-checking --with-build-subdir="$(BUILD_SUBDIR)"
...
SUBDIRS =  intl libiberty opcodes bfd readline zlib libbacktrace libdecnumber libctf binutils gas ld sim gdb gdbserver gprof etc gnulib gdbsupport
TARGET_CONFIGDIRS = 
# This is set by the configure script to the arguments to use when configuring
# directories built for the host system.
HOST_CONFIGARGS =  --cache-file=./config.cache  --with-gnu-as --with-gnu-ld '--prefix=/opt/risc32_self_compile/' '--with-arch=rv32im' '--enable-languages=c' --program-transform-name='s,y,y,' --disable-option-checking
...
TARGET_CONFIGARGS = --cache-file=./config.cache --enable-multilib   '--prefix=/opt/risc32_self_compile/' '--with-arch=rv32im' '--enable-languages=c' --program-transform-name='s,y,y,' --disable-option-checking --with-target-subdir="$(TARGET_SUBDIR)"
# This is the list of variables to export in the environment when
...
AS = as
AR = ar --plugin /usr/lib/gcc/x86_64-pc-linux-gnu/11.2.0/liblto_plugin.so
AR_FLAGS = rc
CC = gcc
CXX = g++
DLLTOOL = dlltool
...
OBJDUMP = objdump
OTOOL = @OTOOL@
RANLIB = ranlib --plugin /usr/lib/gcc/x86_64-pc-linux-gnu/11.2.0/liblto_plugin.so
READELF = readelf
STRIP = strip
WINDRES = windres
WINDMC = windmc
